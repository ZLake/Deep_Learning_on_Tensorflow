...
# -*- coding: utf-8 -*-
"""
Created on Wed Jan 18 11:09:10 2017

@author: lakezhang
"""

# ==============================================================================
# use bigram as input to LSTM cell, 2 letters as one input, embedding used
# ==============================================================================
...
class BiBatchGenerator(object):
    ...
    def _next_batch(self):
        batch = np.zeros(shape=self._batch_size, dtype=np.int) # not one hot, just store idx
        # print 'batch idx %i' %
        for b in range(self._batch_size):
            ...
            batch[b] = ch1 * vocabulary_size + ch2 # the index for the bigram
...
# create the bigram LSTM model with embedding and dropout
def create_lstm_graph_bi(num_nodes, num_unrollings, batch_size, embedding_size=bi_voc_size):
    with tf.Graph().as_default() as g:
        # input to all gates
        sx = tf.Variable(tf.truncated_normal([embedding_size, num_nodes * 4], -0.1, 0.1), name='x')
        # memory of all gates
        sm = tf.Variable(tf.truncated_normal([num_nodes, num_nodes * 4], -0.1, 0.1), name='m')
        # biases all gates
...
graph = create_lstm_graph_bi(64, 10, 32, 128)
bitrain(graph, 4001, 100, 10, 32)
